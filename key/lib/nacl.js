...
(function(nacl) {
    ...
    function crypto_inner_sign(sm, m, n, sk, publicKey) {
        var p = [gf(), gf(), gf(), gf()];
        var i, j, s = new Float64Array(64);
        let messageDigest = new Uint8Array(64), hramDigest = new Uint8Array(64);
        messageDigest = crypto.createHash('sha512')
            .update(sk.subarray(32))
            .update(m)
            .digest();

        let encodedR =  new Uint8Array(32);
        reduce(messageDigest);
        const messageDigestReduced = messageDigest.subarray(0,32);

        scalarbase(p, messageDigestReduced);
        pack(encodedR, p);

        hramDigest = crypto.createHash('sha512')
            .update(encodedR)
            .update(publicKey)
            .update(m)
            .digest();
        ...
        let _sk = sk.subarray(0,32);

        for ( i = 0; i < 64; i++) s[i] = 0;
        for ( i = 0; i < 32; i++) s[i] = messageDigest[i];
        for ( i = 0; i < 32; i++) {
            for ( j = 0; j < 32; j++) {
                s[i+j] += hramDigest[i] * _sk[j];
            }
        }

        modL(sm.subarray(32), s);
        for ( i = 0; i < 32; i++) sm[i] = encodedR[i];
    ...
    nacl.innerSign = function(msg, secretKey, publicKey) {
        checkArrayTypes(msg, secretKey);
        if (secretKey.length !== crypto_sign_SECRETKEYBYTES)
            throw new Error('bad secret key size');
        const _msg = Buffer.from(msg,'hex');
        var signedMsg = new Uint8Array(crypto_sign_BYTES);
        crypto_inner_sign(signedMsg, _msg, _msg.length, secretKey, publicKey);
        return signedMsg;
    };
